	...
	Sentinel.prototype.start = function() {
		...
		for(i = 0; i < this.config.files.length; i += 1) {
			...
			fs.watchFile(file.path, function(curr, prev) {
				...
				// We also need to check if the size has changed
				if(file.processor && self.config.processors.hasOwnProperty(file.processor) && curr.size !== prev.size) {
					// Copy the command
					...
					for(a in file) {
						if(file.hasOwnProperty(a)) {
							command = command.replace(new RegExp('{{' + a + '}}', 'g'), file[a]);
						}
					}
